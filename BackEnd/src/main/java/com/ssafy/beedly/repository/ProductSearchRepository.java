...
public interface ProductSearchRepository extends JpaRepository<PersonalProduct, Long> {
	...
	Slice<PersonalProduct> findPersonalProductByProductNameLike(@Param("productName") String productName, Pageable pageable);

	//----------- 2. userNickName 검색하기
	@Query(value="select p from PersonalProduct p join fetch p.user u where u.userNickname like %:userNickname%")
	Slice<PersonalProduct> findPersonalProductByUserNickname(@Param("userNickname") String userNickname, Pageable pageable);
	...
	Slice<PersonalSearchTag> findPersonalSearchTagByTagId(Long tagId, Pageable pageable);

	//----------- 4. 종료된 작품 검색하기
	@Query(value="select p from PersonalProduct p where  p.artist.id = :id and p.soldStatus = '낙찰'")
	Slice<PersonalProduct> findPersonalProductByTerminated(Long id, Pageable pageable);
