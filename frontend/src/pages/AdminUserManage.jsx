                ...
                {ApplicationList.map((item, idx) =>
                    <div key={idx}>
                        <FlexBox Row_SB style={{ minHeight: "5vh" }}>
                            <div style={{ fontSize: "12px" }}>{item.userName} ({item.userEmail})</div>
                            <FlexBox Row_SB style={{ fontSize: "12px", width: "15vw", marginRight: "3vw" }}>
                                <div onClick={(e) => Approve(item.userId)}>승인</div>
                                <div onClick={(e) => Reject(item.userId)} style={{ color: "red" }}>거부</div>
                            </FlexBox>
                        </FlexBox>
                        <FlexBox Content>
                            <StyledHr
