...
export default function ArtistDetailPage() {
    ...
    const addFavoriteArtist = (e) => {
        ...
        if (userId === User.userId) {
            ...
            if (isMyFavorite) {
                deleteFavoriteArtistApi(favoriteId, (res) => {
                    console.log(res);
                    getInfo();
                }, (err) => {
                    console.log(err);
                ...
                addFavoriteArtistApi(artistId, (res) => {
                    console.log(res);
                    getInfo();
                }, (err) => {
                    console.log(err);
    ...
    const WantUpdateProfile = (e) => {
        ...
        UpdateProfileImgApi(formData, (res) => {
            console.log(res);
            getInfo();
        }, (err) => {
            console.log(err);
                ...
                >
                    ...
                    <ArtistDetailMainInf>
                        <StyledCardArtistImgFrame>
                            ...
                            {(userId === User.userId) ?
                                <div style={{ position: "absolute", width: "30px", height: "30px", paddingLeft: "65px", paddingTop: "60px" }}>
                                    <label htmlFor="image">
                        ...
                        <FlexBox Row_SB style={{ minWidth: "65vw" }}>
                            <div style={{ "fontSize": "16px", "fontWeight": "700", "padding": "46px 0 0 14px" }}>{userNickname}</div>
                        </FlexBox>
                    </ArtistDetailMainInf>
                    <div>
                        {(userId === User.userId) ?
                            <div style={{ position: "absolute", paddingLeft: "90%", paddingTop: "35%" }}>
                                <label htmlFor="bgimage">
                                    <img src={CameraIcon} alt="배경이미지 편집" /> : <div style={{ display: "none" }}></div>
                                </label>
                                <input
                                    id="bgimage"
                                    type="file"
                    ...
                    <ArtistDetailSubInf>
                        {WantUpdateDesc ? <textarea onChange={(e) => { setUpdateDesc(e.target.value); }} value={UpdateDesc} style={{ marginRight: "10px", minHeight: "6vh", minWidth: "65vw", border: "2px solid black", padding: "10px" }} /> : <div style={{ maxWidth: "300px", fontSize: "14px", padding: "10px" }}>{artistDesc}</div>}

                        <FlexBox MainContent2>
                            {(userId === User.userId) && !WantUpdateDesc ? <img src={EditIcon} alt="자기소개 편집" style={{ width: "18px", height: "18px" }} onClick={(e) => setWantUpdateDesc(true)} /> : <div style={{ display: "none" }}></div>}
                            {WantUpdateDesc ? <FlexBox style={{ flexDirection: "column", fontSize: "12px", fontWeight: "bold", minWidth: "55px" }}>
                                <Button XsmallBlack onClick={(e) => SendUpdateDesc()}>저장</Button>
                                <Button XsmallBlack onClick={(e) => { setWantUpdateDesc(false); setUpdateDesc(artistDesc) }}>취소</Button>
                            </FlexBox> : <div></div>}
                        </FlexBox>
                    </ArtistDetailSubInf>
